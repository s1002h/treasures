package com.ancao.treasures.shiro;
...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
		...
		UserBean hasUser = userMapper.getUserByLoginName(token.getUsername());
		// String md5Pwd = new Md5Hash("123", "lucare",2).toString();
		if (hasUser != null) {
			// 若存在，将此用户存放到登录认证info中，无需自己做密码对比，Shiro会为我们进行密码对比校验
			// List<URole> rlist = uRoleDao.findRoleByUid(hasUser.getId());//
			// 获取用户角色
			// List<UPermission> plist =
			// uPermissionDao.findPermissionByUid(hasUser.getId());// 获取用户权限
			// TODO
			URole uRole = new URole();
			uRole.setId(1l);
			uRole.setName("admin");
			List<URole> rlist = new ArrayList<URole>();// 获取用户角色
			rlist.add(uRole);
			// List<UPermission> plist = uPermissionDao.findPermissionByUid();//
			// 获取用户权限
